...
```
Note that (1, 1) in `IDENTITY(1, 1)` represents the seed and increment for the Identity column and defaults to 1. They can be omitted `IDENTITY`.
__TO explicitly supply a value to the identity column__  
First turn on identity insert  
```
SET IDENTITY_INSERT People ON
```  
Specify column list when inserting a record  
```
INSERT INTO People(PersonId, Name) VALUES(1, 'Chukwudi')
```
